// Drop this into /wwwroot/js/dashboards/user/current-location.js
window.Platform = window.Platform || {};
Platform.Dashboards = Platform.Dashboards || {};
Platform.Dashboards.User = Platform.Dashboards.User || {};

Platform.Dashboards.User.CurrentLocation = (function () {
    ...
    async function handleClick(btn) {
        ...
        try {
            ...
            // POST with exact original shape
            ...
            // determine resolved label (server first, fallback to mapbox)
            const resolvedLabel = (respData && typeof respData.label === 'string' && respData.label.trim())
                ? respData.label.trim()
                : (mapbox.label || 'Select Address');

            // Update modal resolved address (try a few selectors used in your UI)
            const resolvedEl = document.querySelector('#resolvedAddress') || document.querySelector('[data-role="resolved-address"]') || document.querySelector('.resolved-address');
            if (resolvedEl) resolvedEl.textContent = resolvedLabel;

            // Update toggle as original code did
            const toggle = document.getElementById('addressToggle');
            if (toggle) toggle.innerHTML = `<i class="fa-solid fa-home"></i> ${resolvedLabel}&nbsp;<i class="fa-solid fa-chevron-down dropdown-icon"></i>`;

            // Close modal if available (original behaviour)
