...
namespace Website.App.Database.Views
{
    ...
    {
        public static DataTable DataTable(long userId = 0, long addressId = 0)
        {
            ...
            if (userId > 0 || addressId > 0)
            {
                ...
                if (userId > 0) { whereclause += $"ua.USER_ID = {userId} "; }
                if (addressId > 0 && userId > 0) { whereclause += "AND "; }
                if (addressId > 0) { whereclause += $"ua.ADDRESS_ID = {addressId}"; }
                sql += whereclause;
            }
